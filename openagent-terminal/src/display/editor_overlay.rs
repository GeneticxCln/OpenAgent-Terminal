...
fn lsp_server_config_for_language(lang: &str) -> Option<openagent_terminal_ide_lsp::ServerConfig> {
    match lang {
        "rust" => Some(openagent_terminal_ide_lsp::ServerConfig { command: "rust-analyzer".into(), args: vec![], initialization_options: None }),
        "typescript" | "javascript" => Some(openagent_terminal_ide_lsp::ServerConfig { command: "typescript-language-server".into(), args: vec!["--stdio".into()], initialization_options: None }),
        "python" => Some(openagent_terminal_ide_lsp::ServerConfig { command: "pyright-langserver".into(), args: vec!["--stdio".into()], initialization_options: None }),
        _ => None,
    }
}
...
impl Display {
    ...
    pub fn editor_overlay_request_completion(&mut self) {
        if let (Some(client), Some(uri), Some(buf)) = (
            ...
            let pos = lsp_types::Position { line: buf.cursor.read().line as u32, character: buf.cursor.read().column as u32 };
            let tdpp = lsp_types::TextDocumentPositionParams { text_document: lsp_types::TextDocumentIdentifier { uri }, position: pos };
            if let Ok(resp) = client.completion(tdpp) {
                ...
                #[cfg(not(feature = "lsp"))]
                {
                    let items: Vec<String> = match resp {
                        lsp_types::CompletionResponse::Array(v) => v.into_iter().map(|i| i.label).collect(),
                        lsp_types::CompletionResponse::List(l) => l.items.into_iter().map(|i| i.label).collect(),
                    };
                    self.editor_overlay.completion_items = items;
                }
                self.editor_overlay.completion_selected = 0;
                self.editor_overlay.completion_active = !self.editor_overlay.completion_items.is_empty();
                self.pending_update.dirty = true;
            }
        }
    }
    pub fn editor_overlay_completion_visible(&self) -> bool { self.editor_overlay.completion_active }
    pub fn editor_overlay_completion_move(&mut self, delta: isize) {
        if self.editor_overlay.completion_items.is_empty() { return; }
    ...
    pub fn editor_overlay_completion_accept(&mut self) {
        if !self.editor_overlay.completion_active { return; }
        #[cfg(feature = "lsp")]
        {
            if let Some(item) = self.editor_overlay.completion_items_full.get(self.editor_overlay.completion_selected).cloned() {
                self.apply_completion_item(item);
            }
        }
        #[cfg(not(feature = "lsp"))]
        {
            if let Some(text) = self.editor_overlay.completion_items.get(self.editor_overlay.completion_selected).cloned() {
                #[cfg(feature = "editor")]
                if let Some(buf) = &self.editor_overlay.buffer { for ch in text.chars() { buf.insert(ch); } }
            }
        }
...
impl EditorOverlayState {
                ...
                Ok(buf) => {
                    ...
                    #[cfg(feature = "lsp")]
                    {
                        ...
                        if let Ok(uri) = lsp_types::Url::from_file_path(&path) {
                            ...
                            if let Some(cfg) = cfg {
                                if let Ok(client) = openagent_terminal_ide_lsp::LspClient::start(&cfg, Some(uri.clone())) {
                                    let text = self.buffer.as_ref().map(|b| b.text()).unwrap_or_default();
    ...
    pub fn save(&self) {
        #[cfg(feature = "editor")]
        if let Some(buf) = &self.buffer { let _ = buf.save(); }
    }
}
    ...
    fn apply_completion_item(&mut self, item: lsp_types::CompletionItem) {
        if let Some(buf) = &self.editor_overlay.buffer {
            ...
            if let Some(edit) = item.text_edit.clone() {
                match edit {
                    lsp_types::CompletionTextEdit::Edit(e) => {
                        ...
                        let text = if item.insert_text_format == Some(lsp_types::InsertTextFormat::SNIPPET) { self.sanitize_snippet(&e.new_text) } else { e.new_text };
                    ...
                    lsp_types::CompletionTextEdit::InsertAndReplace(ir) => {
                        ...
                        let mut rope_mut = buf.rope.write();
                        rope_mut.remove(s..eidx);
                        let text = if item.insert_text_format == Some(lsp_types::InsertTextFormat::SNIPPET) { self.sanitize_snippet(&ir.new_text) } else { ir.new_text };
                        rope_mut.insert(s, &text);
                        inserted = true;
                    },
                }
            ...
            if !inserted {
                ...
                let text = if item.insert_text_format == Some(lsp_types::InsertTextFormat::SNIPPET) { self.sanitize_snippet(&text) } else { text };
                for ch in text.chars() { buf.insert(ch); }
            }
            // Full sync
            if let (Some(client), Some(uri)) = (self.editor_overlay.lsp.as_ref(), self.editor_overlay.lsp_uri.clone()) {
...
impl Display {
    pub fn draw_editor_overlay(&mut self, config: &UiConfig, state: &EditorOverlayState) {
        ...
        let header = if let Some(p) = state.file_path.as_ref() { p.display().to_string() } else { "Untitled".into() };
        self.draw_ai_text(Point::new(start_line, Column(start_col + 2)), tokens.text, tokens.surface, &header, panel_cols.saturating_sub(4));
        ...
        if let Some(buf) = &state.buffer {
            ...
            for (i, raw) in text_all.lines().enumerate().skip(state.scroll_line).take(content_lines) {
                ...
                if text.width() > maxw { // naive clipping by chars
                    let mut acc = String::new();
                    let mut wsum = 0;
                    for ch in text.chars() { let w = ch.width().unwrap_or(1); if wsum + w > maxw { break; } acc.push(ch); wsum += w; }
                    text = acc;
                }
                self.draw_ai_text(Point::new(line, Column(start_col + 2)), tokens.text, tokens.surface, &text, panel_cols.saturating_sub(4));
                ...
                #[cfg(feature = "lsp")]
                {
                    ...
                    for d in &state.diagnostics {
                        ...
                        if (start.line as usize) <= dl && (end.line as usize) >= dl {
                            let seg_start = if start.line as usize == dl { start.character as usize } else { 0 };
                            let seg_end = if end.line as usize == dl { end.character as usize } else { panel_cols.saturating_sub(4) };
                            if seg_start < seg_end {
                                ...
                                let col = match d.severity.unwrap_or(lsp_types::DiagnosticSeverity::HINT) {
                                ...
                                let rect = RenderRect::new(ux, uy, uw, size.cell_height() * 0.08, col, 0.95);
                                let metrics = self.glyph_cache.font_metrics();
                                let size_copy = self.size_info;
        ...
        if state.completion_active && !state.completion_items.is_empty() {
            ...
            for i in 0..popup_lines {
                ...
                if idx == state.completion_selected { 
                    let selbg = RenderRect::new(px, line as f32 * size.cell_height(), pw, size.cell_height(), tokens.surface, 0.92);
                    let rects = vec![selbg];
                    let metrics = self.glyph_cache.font_metrics();
                    let size_copy = self.size_info;
                    self.renderer_draw_rects(&size_copy, &metrics, rects);
                }
                self.draw_ai_text(Point::new(line, Column(start_col + 3)), tokens.text, tokens.surface_muted, &text, popup_cols.saturating_sub(2));
            }
        }
        ...
        if state.references_active && !state.references.is_empty() {
            ...
            for i in 0..popup_lines {
                let idx = i;
                let line = content_top + 1 + i;
                let label = state.references.get(idx).map(|loc| format!("{}:{}:{}", loc.uri.path().rsplit('/').next().unwrap_or(""), loc.range.start.line+1, loc.range.start.character+1)).unwrap_or_default();
                let mut text = label;
                if text.width() > popup_cols - 2 { text.truncate((popup_cols - 2).saturating_sub(1)); }
                ...
                self.draw_ai_text(Point::new(line, Column(start_col + panel_cols - popup_cols - 1)), tokens.text, tokens.surface_muted, &text, popup_cols.saturating_sub(2));
            }
        }
        ...
        if state.rename_active {
            ...
            let size_copy = self.size_info;
            self.renderer_draw_rects(&size_copy, &metrics, rects);
            self.draw_ai_text(Point::new(start_line + 1, Column(start_col + 3)), tokens.text, tokens.surface_muted, &prompt, panel_cols.saturating_sub(6));
        }
        ...
        if state.hover_active && !state.hover_text.is_empty() {
            ...
            for (i, l) in state.hover_text.lines().enumerate() { if i >= lines_to_show { break; } shown.push(l); }
            ...
            let size_copy = self.size_info;
            self.renderer_draw_rects(&size_copy, &metrics, rects);
            self.draw_ai_text(Point::new(start_line + panel_lines.saturating_sub(3), Column(start_col + 3)), tokens.text, tokens.surface_muted, &content, panel_cols.saturating_sub(6));
        }
        ...
        if state.signature_active && !state.signature_label.is_empty() {
            ...
            let px = (start_col + 2) as f32 * size.cell_width();
            let py = (start_line + 1) as f32 * size.cell_height();
            let pw = (label.width().min(panel_cols.saturating_sub(6)) as f32 * size.cell_width()).max(120.0);
            let ph = size.cell_height();
            let bg = RenderRect::new(px, py, pw, ph, tokens.surface_muted, 0.98);
            let rects = vec![bg];
            let metrics = self.glyph_cache.font_metrics();
            let size_copy = self.size_info;
            self.renderer_draw_rects(&size_copy, &metrics, rects);
            self.draw_ai_text(Point::new(start_line + 1, Column(start_col + 3)), tokens.text, tokens.surface_muted, label, panel_cols.saturating_sub(6));
        }
    }
    ...
    pub fn editor_overlay_close(&mut self) { self.editor_overlay.close(); }
    pub fn editor_overlay_save(&mut self) { self.editor_overlay.save(); }

    // Cursor navigation helpers
    #[cfg(feature = "editor")]
    pub fn editor_overlay_move_left(&mut self) { if let Some(buf) = &self.editor_overlay.buffer { buf.move_left(); } self.ensure_cursor_visible(); self.pending_update.dirty = true; }
    #[cfg(feature = "editor")]
    pub fn editor_overlay_move_right(&mut self) { if let Some(buf) = &self.editor_overlay.buffer { buf.move_right(); } self.ensure_cursor_visible(); self.pending_update.dirty = true; }
    #[cfg(feature = "editor")]
    pub fn editor_overlay_move_up(&mut self) { if let Some(buf) = &self.editor_overlay.buffer { buf.move_up(); } self.ensure_cursor_visible(); self.pending_update.dirty = true; }
    #[cfg(feature = "editor")]
    pub fn editor_overlay_move_down(&mut self) { if let Some(buf) = &self.editor_overlay.buffer { buf.move_down(); } self.ensure_cursor_visible(); self.pending_update.dirty = true; }
    #[cfg(feature = "editor")]
    ...
    fn lsp_tdpp_at_cursor(&self) -> Option<lsp_types::TextDocumentPositionParams> {
        ...
        Some(lsp_types::TextDocumentPositionParams { text_document: lsp_types::TextDocumentIdentifier { uri }, position: pos })
    }

    #[cfg(feature = "lsp")]
    pub fn editor_overlay_goto_definition(&mut self) {
        if let (Some(client), Some(tdpp)) = (self.editor_overlay.lsp.as_ref(), self.lsp_tdpp_at_cursor()) {
            if let Ok(resp) = client.definition(tdpp) {
                match resp {
                    lsp_types::GotoDefinitionResponse::Scalar(loc) => self.jump_to_location(loc),
                    lsp_types::GotoDefinitionResponse::Array(mut arr) => if let Some(loc) = arr.pop() { self.jump_to_location(loc) },
                    lsp_types::GotoDefinitionResponse::Link(mut links) => if let Some(link) = links.pop() { self.jump_to_location(lsp_types::Location { uri: link.target_uri, range: link.target_range }) },
                }
            }
        }
    }
    ...
    pub fn editor_overlay_show_references(&mut self) {
        if let (Some(client), Some(tdpp)) = (self.editor_overlay.lsp.as_ref(), self.lsp_tdpp_at_cursor()) {
            ...
            let params = lsp_types::ReferenceParams { text_document_position: tdpp, context, work_done_progress_params: Default::default(), partial_result_params: Default::default() };
    ...
    pub fn editor_overlay_references_move(&mut self, delta: isize) {
        if !self.editor_overlay.references_active || self.editor_overlay.references.is_empty() { return; }
    ...
    pub fn editor_overlay_references_accept(&mut self) {
        if !self.editor_overlay.references_active { return; }
        if let Some(loc) = self.editor_overlay.references.get(self.editor_overlay.references_selected).cloned() {
    ...
    pub fn editor_overlay_toggle_rename(&mut self) {
        ...
        if self.editor_overlay.rename_active { self.editor_overlay.rename_text.clear(); }
        self.pending_update.dirty = true;
    }

    #[cfg(feature = "lsp")]
    pub fn editor_overlay_rename_input(&mut self, ch: char) { self.editor_overlay.rename_text.push(ch); self.pending_update.dirty = true; }
    #[cfg(feature = "lsp")]
    pub fn editor_overlay_rename_backspace(&mut self) { self.editor_overlay.rename_text.pop(); self.pending_update.dirty = true; }

    #[cfg(feature = "lsp")]
    pub fn editor_overlay_rename_commit(&mut self) {
        if !self.editor_overlay.rename_active { return; }
        let new_name = self.editor_overlay.rename_text.clone();
        if new_name.is_empty() { self.editor_overlay.rename_active = false; self.pending_update.dirty = true; return; }
        if let (Some(client), Some(tdpp)) = (self.editor_overlay.lsp.as_ref(), self.lsp_tdpp_at_cursor()) {
            let params = lsp_types::RenameParams { text_document_position: tdpp, new_name, work_done_progress_params: Default::default() };
    ...
    fn apply_workspace_edit(&mut self, edit: lsp_types::WorkspaceEdit) {
        if let Some(changes) = edit.changes {
            for (uri, edits) in changes {
                if let Some(cur_uri) = self.editor_overlay.lsp_uri.clone() { if uri != cur_uri { continue; } }
                if let Some(buf) = &self.editor_overlay.buffer {
    ...
    #[cfg(feature = "lsp")]
    fn lsp_pos_to_char_index_buf(&self, buf: &openagent_terminal_ide_editor::EditorBuffer, pos: lsp_types::Position) -> usize {
    ...
    pub fn editor_overlay_format_document(&mut self) {
        if let (Some(client), Some(uri), Some(buf)) = (self.editor_overlay.lsp.as_ref(), self.editor_overlay.lsp_uri.clone(), self.editor_overlay.buffer.as_ref()) {
            let options = lsp_types::FormattingOptions { tab_size: 4, insert_spaces: true, ..Default::default() };
            let params = lsp_types::DocumentFormattingParams { text_document: lsp_types::TextDocumentIdentifier { uri }, options, work_done_progress_params: Default::default() };
            if let Ok(edits) = client.formatting(params) {
                let mut rope = buf.rope.write();
                // Apply edits back-to-front
                let mut ranges: Vec<(usize, usize, String)> = Vec::new();
                for e in edits { let s = self.lsp_pos_to_char_index_buf(buf, e.range.start); let eidx = self.lsp_pos_to_char_index_buf(buf, e.range.end); ranges.push((s,eidx,e.new_text)); }
                ranges.sort_by(|a,b| b.0.cmp(&a.0));
                for (s,eidx,txt) in ranges { rope.remove(s..eidx); rope.insert(s,&txt); }
                // Full sync after
                #[cfg(feature = "lsp")]
                {
                    let version = buf.meta.read().version;
                    let change = lsp_types::TextDocumentContentChangeEvent { range: None, range_length: None, text: rope.to_string() };
                    let _ = client.change_document(self.editor_overlay.lsp_uri.clone().unwrap(), version, vec![change]);
                }
                self.pending_update.dirty = true;
            }
        }
        ...
        if let (Some(client), Some(tdpp)) = (self.editor_overlay.lsp.as_ref(), self.lsp_tdpp_at_cursor()) {
            if let Ok(Some(hover)) = client.hover(tdpp) {
                ...
                let text = match hover.contents {
                    lsp_types::HoverContents::Scalar(ms) => match ms { lsp_types::MarkedString::String(s) => s, lsp_types::MarkedString::LanguageString(ls) => ls.value },
                    lsp_types::HoverContents::Array(arr) => arr.into_iter().map(|m| match m { lsp_types::MarkedString::String(s) => s, lsp_types::MarkedString::LanguageString(ls) => ls.value }).collect::<Vec<_>>().join("\n"),
    ...
    pub fn editor_overlay_signature_help(&mut self) {
        if let (Some(client), Some(tdpp)) = (self.editor_overlay.lsp.as_ref(), self.lsp_tdpp_at_cursor()) {
            if let Ok(Some(sig)) = client.signature_help(tdpp) {
                if let Some(item) = sig.signatures.first() { self.editor_overlay.signature_label = item.label.clone(); self.editor_overlay.signature_active = true; self.pending_update.dirty = true; }
            }
        }
    }
    ...
    pub fn editor_overlay_insert_char(&mut self, ch: char) {
        if let Some(buf) = &self.editor_overlay.buffer { buf.insert(ch); }
    ...
    pub fn editor_overlay_backspace(&mut self) {
        if let Some(buf) = &self.editor_overlay.buffer { buf.backspace(); }
        #[cfg(feature = "lsp")]
        {
            if let (Some(client), Some(uri), Some(buf)) = (
                self.editor_overlay.lsp.as_ref(),
