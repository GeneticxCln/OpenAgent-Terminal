...
source_split: PaneId,
    ...
    pub fn update_position(&mut self, pos: (f32, f32)) -> bool {
        ...
        if !self.is_active {
            let distance = ((pos.0 - self.start_pos.0).powi(2) + 
                           (pos.1 - self.start_pos.1).powi(2)).sqrt();
    ...
    pub fn update_animations(&mut self) -> bool {
        ...
        if let Some(ref mut drag_state) = self.active_drag {
            if drag_state.is_active {
                ...
                drag_state.ghost_pane_alpha = (self.ghost_alpha * (1.0 + (elapsed * 0.003).sin() * 0.1))
                    .clamp(0.3, 0.7);
                
                // Animate drop zone highlight
                let target_highlight = if drag_state.current_drop_zone.is_some() { 1.0 } else { 0.0 };
...
split_areas: &[(TabId, PaneId, f32, f32, f32, f32)], // (tab_id, split_id, x, y, w, h)
    ...
    pub fn get_visual_effects(&self) -> Option<PaneDragVisualEffects> {
        self.active_drag.as_ref().map(|drag| {
            PaneDragVisualEffects {
...
let split_id = PaneId(1);
    ...
    fn test_calculate_drop_zone() {
        ...
        let tab_positions = vec![
            (TabId(1), 0.0, 100.0),
            (TabId(2), 100.0, 100.0),
        ];
        
        // Test tab drop zone
        let drop_zone = manager.calculate_drop_zone(
            (50.0, 10.0),
            &tab_positions,
            &[]
        );
