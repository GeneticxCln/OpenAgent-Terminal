...
use openagent_terminal_core::grid::{Dimensions, Indexed};
use openagent_terminal_core::index::{Column, Line, Point};
use openagent_terminal_core::selection::SelectionRange;
use openagent_terminal_core::term::cell::{Cell, Flags, Hyperlink};
use openagent_terminal_core::term::search::{Match, RegexSearch};
use openagent_terminal_core::term::{self, RenderableContent as TerminalContent, Term, TermMode};
use openagent_terminal_core::vte::ansi::{Color, CursorShape, NamedColor};
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            if self.cursor_point == cell.point {
                ...
                // Do not mutate the cell for block cursor; the cursor is rendered via shader overlay.
    ...
    fn new(content: &mut RenderableContent<'_>, cell: Indexed<&Cell>) -> Self {
            ...
            .and_then(|hint| hint.advance(viewport_start, num_cols, cell.point))
        {
            ...
            if content.config.debug.theme_selection {
                ...
                bg_alpha = 0.0; // Selection background drawn via shader overlay
            } else {
                ...
                if fg == bg && !cell.flags.contains(Flags::HIDDEN) {
                    ...
                    // bg_alpha intentionally suppressed; background drawn via overlay
                }
