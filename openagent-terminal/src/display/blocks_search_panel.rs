...
pub use crate::display::blocks_search_actions::{ActionsMenuState, BlockAction, HelpOverlayState};
...
impl Display {
    ...
    pub fn draw_blocks_search_overlay(&mut self, config: &UiConfig, state: &BlocksSearchState) {
        if !state.active {
            return;
        }
    ...
    fn draw_search_header(
        ...
        if state.has_active_filters() {
            ...
            self.draw_ai_text(
                Point::new(line, Column(indicator_col)),
                accent_fg,
                bg,
                filter_indicator,
                2,
    ...
    fn draw_search_input(
        ...
        self.draw_ai_text(
            Point::new(line, Column(mode_prefix.width())),
            fg,
            bg,
            &state.query,
            num_cols - mode_prefix.width(),
        );
    ...
    fn draw_actions_menu(&mut self, menu: &ActionsMenuState, tokens: &ThemeTokens) {
        if !menu.active {
            return;
        }
        ...
        let menu_width = 30;
        let menu_height = 15;
        let menu_x = (menu.position.column.0 as f32)
            .min(size_info.width() - menu_width as f32 * size_info.cell_width());
        let menu_y = (menu.position.line as f32 * size_info.cell_height())
            .min(size_info.height() - menu_height as f32 * size_info.cell_height());

        // Menu background
        let menu_bg = RenderRect::new(
            menu_x,
            menu_y,
            menu_width as f32 * size_info.cell_width(),
            menu_height as f32 * size_info.cell_height(),
            tokens.surface,
            0.95,
        );
        let border_rect = RenderRect::new(
            menu_x - 1.0,
            menu_y - 1.0,
            (menu_width as f32 + 2.0) * size_info.cell_width(),
            (menu_height as f32 + 2.0) * size_info.cell_height(),
            tokens.border,
            1.0,
        );
    ...
    fn draw_help_overlay(&mut self, help: &HelpOverlayState, tokens: &ThemeTokens) {
        if !help.active {
            return;
        }
