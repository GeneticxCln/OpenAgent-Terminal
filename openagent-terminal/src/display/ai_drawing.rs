...
impl<'a> AiDrawContext<'a> {
    ...
    fn draw_panel_content(
        ...
        if current_line <= separator_line {
            self.draw_header(ai_state, current_line, fg, bg, num_cols);
            current_line += 1;
        }
    ...
    fn draw_action_hints(&mut self, line: usize, bg: Rgb, num_cols: usize) {
        let actions = match self.mode {
            ...
            AiRenderMode::Overlay => "[Tab] Accept  [Ctrl+C] Copy  [Esc] Dismiss",
        };
    ...
    /// Draw error state
    fn draw_error_state(&mut self, error: &str, line: usize, _fg: Rgb, bg: Rgb, num_cols: usize) {
