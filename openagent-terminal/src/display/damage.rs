...
mod tests {
    ...
    fn render_iterator_merges_adjacent_lines() {
        ...
        let left = 2;
        let len = 20; // columns
        tracker.frame().damage_line(LineDamageBounds::new(5, left, len));
        tracker.frame().damage_line(LineDamageBounds::new(6, left, len));

        let size_info: SizeInfo<u32> =
            SizeInfo::new(800.0, 600.0, 10.0, 20.0, 0.0, 0.0, true).into();
