...
fn test_tab_bar_offscreen_smoke_snapshot() {
    ...
    let output =
        std::process::Command::new(&bin).output().expect("failed to run wgpu_snapshot example");
    assert!(
    ...
    let snapshot =
        image::open(img_path).expect("missing offscreen snapshot from example").to_rgba8();
