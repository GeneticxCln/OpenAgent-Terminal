...
mod integration_tests {
    use openagent_terminal::ai::agents::{
        ...
        privacy_content_filter::{PrivacyContentFilter, PrivacyFilterConfig, DataClassification, ComplianceStandard},
        ...
        workflow_orchestrator::{WorkflowOrchestrator, WorkflowConfig},
    ...
    async fn test_conversation_with_privacy_filtering() -> Result<()> {
        ...
        let session_id = conversation_manager
            .create_session(Some("Integration Test Session".to_string())).await?;

        // Test sensitive content handling
        let sensitive_message = "My email is test@example.com and my credit card is 4111-1111-1111-1111";
        ...
        // Apply redaction
        let redaction_result = privacy_filter.redact_content(
            sensitive_message,
            "gdpr-policy", 
            None
        ).await?;
        ...
        let conversation_history = conversation_manager.get_conversation_history(session_id, 10).await?;
    ...
    async fn test_advanced_features_integration() -> Result<()> {
        ...
        let session_id = conversation_manager
            .create_session(Some("Advanced Features Test".to_string())).await?;

        // Test goal creation and management
        let goal_id = advanced_features.create_goal(
            "Test Goal".to_string(),
        ...
        let branch_id = advanced_features.create_conversation_branch(
            session_id,
            Some("Test Branch".to_string())
        ).await?;

        let branches = advanced_features.get_conversation_branches(session_id).await?;
        assert!(!branches.is_empty());
        assert!(branches.iter().any(|b| b.id == branch_id));

        // Test context switching
        advanced_features.switch_conversation_context(session_id, branch_id).await?;

        // Update goal progress
        advanced_features.update_goal_progress(goal_id.clone(), 0.5).await?;
    ...
    async fn test_terminal_ui_integration() -> Result<()> {
        ...
        let session_id = terminal_ui.create_conversation_session(
            Some("UI Test Session".to_string())
        ).await?;
        assert!(!session_id.is_empty());

        // Test theme and layout management
        use openagent_terminal::ai::agents::terminal_ui_integration::{Theme, Layout};
        
        terminal_ui.set_theme(Theme::dark()).await?;
        terminal_ui.set_layout(Layout::SplitVertical).await?;

        // Test privacy status updates
        terminal_ui.update_privacy_status(
            0.7,
            3,
            vec!["GDPR".to_string(), "CCPA".to_string()]
        ).await?;
    ...
    async fn test_workflow_orchestration_integration() -> Result<()> {
        ...
        use openagent_terminal::ai::agents::workflow_orchestrator::{
            WorkflowDefinition, WorkflowStep, StepType, WorkflowPriority
        };
        ...
        // Execute workflow
        let execution_id = workflow_orchestrator.execute_workflow(
            workflow_def,
        ...
        let status = workflow_orchestrator.get_execution_status(execution_id).await?;
    ...
    async fn test_project_context_integration() -> Result<()> {
        ...
        // Test file analysis
        let files = project_context.get_project_files(&project_info.root_path, 100).await?;
        assert!(!files.is_empty());

        // Test dependency analysis
        let dependencies = project_context.analyze_dependencies(&project_info.root_path).await?;
        // Dependencies might be empty for some projects, so just verify it doesn't error

        // Test project insights generation
        let insights = project_context.generate_project_insights(&project_info.root_path).await?;
    ...
    async fn test_natural_language_processing() -> Result<()> {
        ...
        let session_id = conversation_manager
            .create_session(Some("NLP Test Session".to_string())).await?;

        // Test natural language processing with privacy filtering
        let user_input = "Help me set up a database connection with username admin and password secret123";
        ...
        let response = natural_language.process_message(&processed_input, None).await?;
        ...
        let history = conversation_manager.get_conversation_history(session_id, 10).await?;
    ...
    async fn test_comprehensive_integration_flow() -> Result<()> {
        ...
        let session_id = conversation_manager
            .create_session(Some("Comprehensive Integration Test".to_string())).await?;

        // 2. Set up UI with session
        terminal_ui.add_conversation_message(
            session_id,
            "System".to_string(),
        ...
        let nl_response = natural_language.process_message(&redaction_result.redacted_content, None).await?;
        ...
        advanced_features.update_goal_progress(goal_id.clone(), 0.8).await?;

        // 8. Generate compliance report
        let date_range = (chrono::Utc::now() - chrono::Duration::hours(1), chrono::Utc::now());
        let compliance_report = privacy_filter.generate_compliance_report(
    ...
    async fn test_error_handling_and_resilience() -> Result<()> {
        ...
        let result = conversation_manager.get_conversation_history(invalid_session, 10).await;
...
mod no_ai_tests {
    ...
    fn ai_feature_disabled() {
        ...
        assert!(true, "AI features are disabled, skipping integration tests");
    }
}