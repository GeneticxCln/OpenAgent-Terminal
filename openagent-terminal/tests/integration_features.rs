...
mod blocks_tests {
    use super::*;
    use openagent_terminal::command_history::{CommandHistory, HistoryEntry};
...
mod security_tests {
    use super::*;
    use openagent_terminal::security_config::{SecurityConfig, SecurityLensFactory, CustomSecurityPattern};
...
mod integration_tests {
    ...
    async fn test_dangerous_command_flow() {
        ...
        let security_result = SecurityLensFactory::test_command(&config, dangerous_command).unwrap();
        assert_ne!(security_result.risk_level, "Safe");
        assert!(security_result.requires_confirmation);
...
mod performance_tests {
    ...
    async fn test_blocks_storage_performance() {
        ...
        assert!(search_duration.as_millis() < 1000, "Search took {}ms", search_duration.as_millis());
    }
}
