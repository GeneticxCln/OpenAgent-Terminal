...
async fn main() -> anyhow::Result<()> {
    ...
    let workflow_id = workflow_orchestrator
        .create_workflow("project-analysis-and-codegen", workflow_context, None)
        .await?;
...
async fn create_project_analysis_workflow_template() -> anyhow::Result<WorkflowTemplate> {
    let template = WorkflowTemplate {
        ...
        description: "Comprehensive workflow that analyzes project structure and generates code"
            .to_string(),
        ...
        steps: vec![
            ...
            WorkflowStep {
                ...
                output_mapping: HashMap::from([(
                    "success".to_string(),
                    "conversation_updated".to_string(),
                )]),
                parallel_group: None,
            },
        ],
...
async fn monitor_workflow_progress(
    ...
    loop {
        ...
        let current_completed = workflow
            .steps
            .iter()
        ...
        if current_completed > completed_steps {
            let newly_completed: Vec<_> = workflow
                .steps
                .iter()
                .filter(|s| matches!(s.status, StepExecutionStatus::Completed))
...
async fn demonstrate_workflow_templates(orchestrator: &WorkflowOrchestrator) -> anyhow::Result<()> {
    ...
    println!(
        "  1. project-analysis-and-codegen - Comprehensive project analysis and code generation"
    );
