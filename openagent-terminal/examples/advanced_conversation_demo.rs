use openagent_terminal::ai::agents::{
    advanced_conversation_features::{
        AdvancedConversationFeatures, AdvancedConversationConfig, BranchReason, SummaryType,
        MergeStrategy, ContextSharingLevel,
    },
    conversation_manager::ConversationManager,
    blitzy_project_context::BlitzyProjectContextAgent,
    workflow_orchestrator::WorkflowOrchestrator,
    natural_language::{ConversationRole, Entity, Intent, EntityType},
    Agent, AgentConfig,
};
use std::sync::Arc;
use chrono::Duration;
...
async fn main() -> Result<()> {
    ...
    // Create main session
    let main_session = conversation_manager.create_session(Some("Main Development Session".to_string())).await?;
    ...
    let jwt_branch = advanced_features.create_branch(
        main_session,
    ...
    let oauth_branch = advanced_features.create_branch(
        main_session,
    ...
    let brief_summary = advanced_features.generate_summary(main_session, SummaryType::Brief).await?;
    ...
    let comprehensive_summary = advanced_features.generate_summary(main_session, SummaryType::Comprehensive).await?;
    println!("📄 Comprehensive Summary: {}", comprehensive_summary.content);
    
    let action_summary = advanced_features.generate_summary(main_session, SummaryType::ActionFocused).await?;
    ...
    let auth_goal = advanced_features.start_goal_tracking(
        main_session,
    ...
    let testing_goal = advanced_features.start_goal_tracking(
        main_session,
    ...
    let frontend_session = conversation_manager.create_session(Some("Frontend Integration".to_string())).await?;
    let testing_session = conversation_manager.create_session(Some("Testing Strategy".to_string())).await?;
    let docs_session = conversation_manager.create_session(Some("Documentation".to_string())).await?;
    ...
    let auth_group = advanced_features.create_session_group(
    ...
    let merge_id = advanced_features.merge_branches(
        main_session,
    ...
    println!("🏥 Advanced Features Health: {}", if status.is_healthy { "✅ Healthy" } else { "❌ Unhealthy" });
    ...
    let final_summary = advanced_features.generate_summary(main_session, SummaryType::GoalOriented).await?;
