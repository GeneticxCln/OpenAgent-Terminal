use openagent_terminal::ai::agents::{
    privacy_content_filter::{
        PrivacyContentFilter, PrivacyFilterConfig, DataClassification, ComplianceStandard,
        PrivacyPolicy, RedactionRule, RedactionMethod, SensitivityLevel, UserContext,
    },
...
async fn main() -> Result<()> {
    ...
    let test_contents = vec![
        ("Email Content", "Please contact us at john.doe@company.com or call 555-123-4567 for assistance."),
        ...
        ("Health Data", "Patient ID: 12345, Diagnosis: Diabetes Type 2, Insurance: Blue Cross 987654321"),
        ("Mixed Content", "John Smith (john@email.com, 555-0123) has CC: 4111-1111-1111-1111 exp 12/25"),
    ];
    ...
    for (content_type, content) in &test_contents {
        ...
        for detection in &scan_result.detections {
            println!("  🚨 Detection: {} (confidence: {:.2}, sensitivity: {:?})", 
    ...
    let healthcare_result = privacy_filter.redact_content(
        healthcare_content,
        "healthcare-policy",
        Some(&user_context)
    ).await?;
    ...
    let session_id = conversation_manager
        .create_session(Some("Privacy Demo Session".to_string())).await?;
    ...
    for (i, message) in conversation_messages.iter().enumerate() {
        ...
        if !scan_result.detections.is_empty() {
            println!("🚨 Privacy issues detected! Risk score: {:.2}", scan_result.overall_risk_score);
