...
impl GitContextPlugin {
    ...
    fn is_git_repo(&self, path: &Path) -> bool {
        path.join(".git").exists() || self.run_git_command(&["rev-parse", "--git-dir"], path).is_ok()
    }
    ...
    fn get_branch(&mut self, path: &Path) -> Option<String> {
        ...
        let branch = self.run_git_command(&["branch", "--show-current"], path)
            .ok()
            .filter(|b| !b.is_empty())
            .or_else(|| {
        ...
        if let Ok(output) = self.run_git_command(&["remote", "-v"], path) {
            for line in output.lines() {
                ...
                if parts.len() >= 2 {
                    ...
                    if parts.len() >= 3 {
                        remote.insert("type".to_string(), parts[2].trim_matches('(').trim_matches(')').to_string());
                    }
                    remotes.push(remote);
                }
            }
