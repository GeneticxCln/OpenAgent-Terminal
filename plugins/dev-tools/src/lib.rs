    ...
    fn update_cache(&mut self) {
        ...
        if let Ok(output) = self.run_command("pip", &["list", "--format=json"]) {
            if let Ok(packages) = serde_json::from_str::<Vec<serde_json::Value>>(&output) {
                self.cache.pip_packages = packages.iter()
        ...
        if let Ok(output) = self.run_command("kubectl", &["get", "namespaces", "-o", "jsonpath={.items[*].metadata.name}"]) {
...
impl Plugin for DevToolsPlugin {
    ...
    fn init(&mut self, config: PluginConfig) -> Result<(), PluginError> {
        ...
        self.python_version = self.run_command("python", &["--version"]).ok()
            .or_else(|| self.run_command("python3", &["--version"]).ok());
        self.kubectl_version = self.run_command("kubectl", &["version", "--client", "--short"]).ok();
    ...
    fn collect_context(&self, request: ContextRequest) -> Option<Context> {
        ...
        if request.purpose.contains("python") || request.purpose.contains("all") {
            ...
            if let Ok(requirements) = fs::read_to_string("requirements.txt") {
                let packages: Vec<String> = requirements.lines()
        ...
        // Kubernetes context
        if request.purpose.contains("kubernetes") || request.purpose.contains("k8s") || request.purpose.contains("all") {
        ...
        if request.purpose.contains("system") || request.purpose.contains("monitoring") || request.purpose.contains("all") {
    ...
    fn execute_command(&self, cmd: &str, _args: &[String]) -> Result<CommandOutput, PluginError> {
        ...
        match cmd {
            ...
            "k8s-status" => {
                ...
                if self.kubectl_version.is_none() {
                    return Ok(CommandOutput {
                        stdout: "kubectl not found. Please install kubectl to use Kubernetes features.".to_string(),
                        stderr: String::new(),
                        exit_code: 1,
                        execution_time_ms: start.elapsed().as_millis() as u64,
                    });
                ...
                if let Ok(cluster_info) = self.run_command("kubectl", &["cluster-info", "--request-timeout=2s"]) {
                    output.push_str("\n🌐 Cluster Info:\n");
                ...
                // Pod status
                if let Ok(pods) = self.run_command("kubectl", &["get", "pods", "--all-namespaces", "--no-headers"]) {
                    let pod_count = pods.lines().count();
                    let running = pods.lines().filter(|l| l.contains("Running")).count();
                    output.push_str(&format!("📦 Pods: {} total ({} running)\n", pod_count, running));
                }
            ...
            "system-stats" => {
                ...
                output.push_str(&format!("  1 min:  {}\n", stats.get("load_1min").unwrap_or(&"N/A".to_string())));
                output.push_str(&format!("  5 min:  {}\n", stats.get("load_5min").unwrap_or(&"N/A".to_string())));
                output.push_str(&format!("  15 min: {}\n", stats.get("load_15min").unwrap_or(&"N/A".to_string())));
                
                // Memory
                if let (Some(total), Some(available)) = (stats.get("memtotal"), stats.get("memavailable")) {
