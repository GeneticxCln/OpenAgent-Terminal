...
impl WasmSandbox {
    ...
    pub fn execute_plugin(&mut self, plugin_id: &str, function: &str, _args: &[u8]) -> RuntimeResult<Vec<u8>> {
        ...
        if function.starts_with("net:") && self.config.sandbox_network {
            return Err(RuntimeError::SandboxViolation(
                "Network access denied by sandbox".into(),
            ));
        }
        ...
        if function.starts_with("host:") {
            let allowed = matches!(
                function,
                "host:log" | "host:emit_event" | "host:version"
            );
            if !allowed {
                return Err(RuntimeError::SandboxViolation(
                    format!("Host API '{}' is not permitted", function),
                ));
            }
...
mod tests {
    ...
    fn sandbox_fs_network_denied_by_default() {
        ...
        assert!(matches!(err_fs, Err(RuntimeError::SandboxViolation(msg)) if msg.contains("Filesystem")));

        let err_net = sb.execute_plugin("p1", "net:connect", b"");
        assert!(matches!(err_net, Err(RuntimeError::SandboxViolation(msg)) if msg.contains("Network")));
    }
    ...
    fn host_api_boundaries_enforced() {
        ...
        assert!(matches!(err, Err(RuntimeError::SandboxViolation(msg)) if msg.contains("not permitted")));
    }
}
