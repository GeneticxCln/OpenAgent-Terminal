use crate::config::{Theme, ThemePackage, PackageManifest, ThemeAsset};
...
pub fn export_theme_with_assets(theme: &Theme, assets: Vec<ThemeAsset>, output_path: &Path) -> Result<()> {
    let package = ThemePackage {
        theme: theme.clone(),
        assets: Some(assets),
        manifest: create_manifest(theme)?,
