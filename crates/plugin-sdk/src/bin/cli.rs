...
fn main() {
    ...
    match args[1].as_str() {
        ...
        "sign" => {
            if args.len() < 4 {
                eprintln!("Usage: plugin-sdk-cli sign <priv_key_hex|file> <wasm> [--out <sigfile>]");
                return;
            }
            let key_arg = &args[2];
            let wasm = &args[3];
...
fn print_help() {
    ...
    println!("    sign <priv|file> <wasm> [--out file]  Create <wasm>.sig using the private key (ed25519, hex)");
...
fn copy_to_dir(src: &std::path::Path, dest_dir: &std::path::Path) -> std::io::Result<std::path::PathBuf> {
...
fn install_plugin(src: &str) -> anyhow::Result<()> {
    ...
    if src.starts_with("http://") || src.starts_with("https://") {
        ...
        if !path.exists() { anyhow::bail!("Source not found: {}", src); }
...
fn add_trusted_key(arg: &str) -> anyhow::Result<()> {
    ...
    if key_hex.len() != 64 { // 32 bytes hex
...
fn sign_wasm(key_arg: &str, wasm: &str, sig_out: Option<&str>) -> anyhow::Result<std::path::PathBuf> {
    use ed25519_dalek::SigningKey;
    use sha2::{Digest, Sha256};

    let key_hex = if std::path::Path::new(key_arg).exists() { std::fs::read_to_string(key_arg)? } else { key_arg.to_string() };
    let key_bytes = hex::decode(key_hex.trim())?;
    if key_bytes.len() != 32 { anyhow::bail!("expected 32-byte ed25519 private key in hex"); }

    let wasm_path = std::path::Path::new(wasm);
    let wasm_bytes = std::fs::read(wasm_path)?;
    let digest = Sha256::digest(&wasm_bytes);

    let sk = SigningKey::from_bytes(&key_bytes.clone().try_into().map_err(|_| anyhow::anyhow!("invalid key length"))?);
    let sig = sk.sign(&digest);
    let sig_hex = hex::encode(sig.to_bytes());
    let out_path = sig_out.map(std::path::PathBuf::from).unwrap_or_else(|| wasm_path.with_extension("sig"));
...
fn verify_signature(wasm: &str, sig_file: Option<&str>) -> anyhow::Result<bool> {
    ...
    if !sig_path.exists() { anyhow::bail!("Signature file not found: {}", sig_path.display()); }
    ...
    if let Some(cfg) = dirs::config_dir() {
        ...
        for entry in std::fs::read_dir(dir)? {
            let entry = entry?;
            if entry.path().extension().and_then(|s| s.to_str()) != Some("pub") { continue; }
