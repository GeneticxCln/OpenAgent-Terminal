...
async fn test_host_execute_command_permission_denied() {
    ...
    let mgr =
        PluginManager::with_host_and_dirs(vec![temp.path().to_path_buf()], Some(host)).unwrap();
...
async fn test_host_execute_command_allowed() {
    ...
    let host = Arc::new(MockHost::new());
    let mgr =
        PluginManager::with_host_and_dirs(vec![temp.path().to_path_buf()], Some(host.clone()))
            .unwrap();
