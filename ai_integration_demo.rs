...
// Simplified versions of our core types for this demo
        ...
        let response_content = match event {
            TerminalEventType::CommandFailed { command, error, .. } => {
                if command.contains("git") && error.contains("not a git repository") {
                    "It looks like you're trying to run a Git command, but this directory isn't a Git repository. Try running `git init` to initialize a new repository.".to_string()
                } else if command.contains("npm") && error.contains("not found") {
                    "NPM command failed because Node.js/npm is not installed. Try installing Node.js from https://nodejs.org/".to_string()
                } else if command.contains("python") && error.contains("No module named") {
                    "Python module not found. You may need to install it with `pip install <module-name>` or activate the correct virtual environment.".to_string()
                } else {
                    format!("Command '{}' failed. Error: {}. Try checking the command syntax or required dependencies.", command, error)
                }
            }
            TerminalEventType::DirectoryChanged { new_path, .. } => {
                if new_path.join(".git").exists() {
                    "You're now in a Git repository. Try `git status` to see the current state.".to_string()
            ...
            _ => "I'm ready to help with any terminal issues you encounter.".to_string(),
        };
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let mut terminal_bridge = TerminalEventBridge::new(Arc::clone(&integrator_arc), current_dir);
...
// Mock implementations to simulate the .git, package.json, and Cargo.toml detection
impl PathBuf {
    fn exists_mock(&self, file: &str) -> bool {
        // Mock implementation for demo
        if self.to_string_lossy().contains("git") && file == ".git" {
            true
        } else if self.to_string_lossy().contains("node") && file == "package.json" {
            true
        } else if self.to_string_lossy().contains("rust") && file == "Cargo.toml" {
            true
        } else {
            false
        }
...
// Helper trait extension for the demo
trait PathExt {
    fn exists(&self) -> bool;
}
...
impl PathExt for PathBuf {
    fn exists(&self) -> bool {
        // For demo purposes, we'll mock this based on path name
        self.exists_mock(".git") || self.exists_mock("package.json") || self.exists_mock("Cargo.toml")
    }
}